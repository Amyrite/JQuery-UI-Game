$(document).ready(function() {
    ...
    $(document).on('click', '#next', function() {
        ...
        window.location.href = "play3.html";
    });
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response !== 'Intermediate') {
                updateRankToIntermediate(username);
            }
        },
        error: function(xhr, status, error) {
...
function updateRankToElementary(username) {
    $.ajax({
        type: 'POST',
        url: 'updateRanktoIntermediate.php',
        data: { username: username },
        success: function(response) {
            console.log('Rank updated to Intermediate successfully.');
            document.getElementById('rank').textContent = 'Intermediate';
        },
        error: function(xhr, status, error) {
            console.error('Error updating rank to Intermediate:', error);
        }
    });
}
