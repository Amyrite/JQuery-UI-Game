$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(xml) {
            ...
            $(xml).find('word').each(function() {
                ...
                if (difficultyValue === 'Advanced') {
                    // Create card element
                    var card = $('<div class="card" data-part-of-speech="' + partOfSpeechValue + '">' + hanziValue + '<br>' + pinyinValue + '</div>');
    ...
    $(document).on('click', '#next', function() {
        ...
        window.location.href = "play5.html";
    });
...
function updateRankToExpert(username) {
    $.ajax({
        type: 'POST',
        url: 'updateRanktoExpert.php',
        data: { username: username },
        success: function(response) {
            console.log('Rank updated to Expert successfully.');
            document.getElementById('rank').textContent = 'Expert';
        },
        error: function(xhr, status, error) {
            console.error('Error updating rank to Expert:', error);
        }
    });
}